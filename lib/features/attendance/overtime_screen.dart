...
import 'widgets/overtime_dialog.dart';

class OvertimeScreen extends StatefulWidget {
  final int userId;

  const OvertimeScreen({super.key, required this.userId});
...
class _OvertimeScreenState extends State<OvertimeScreen> {
  ...
  UserModel? _user;
  List<OvertimeRequestModel> _allRequests = [];
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final requests = await _api.getOvertimeRequests(widget.userId);
      setState(() {
        _user = user;
      ...
      ShadToaster.of(context).show(
        const ShadToast.destructive(
          title: Text('Error'),
          description: Text('Failed to load overtime data.'),
        ),
      );
    }
  }

  List<OvertimeRequestModel> get _filteredRequests {
    if (!_showTodayOnly) return _allRequests;
    final now = DateTime.now();
    return _allRequests.where((r) {
      return r.requestDate.year == now.year &&
          r.requestDate.month == now.month &&
          r.requestDate.day == now.day;
    }).toList();
  }

  void _showFileDialog([OvertimeRequestModel? request]) {
  ...
  Future<void> _handleDelete(int overtimeId) async {
    final success = await _api.deleteOvertimeRequest(overtimeId);
    if (success) {
      _loadData();
      ShadToaster.of(context).show(
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            _showTodayOnly
                ? 'No overtime requests for today'
  ...
  Widget _buildRequestCard(OvertimeRequestModel item) {
    return Padding(
      ...
      child: ShadCard(
        ...
        child: Column(
          children: [
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Text(
                        DateFormat('MMMM dd, yyyy').format(item.requestDate),
                        style: const TextStyle(
                  ...
                  Row(
                    children: [
                      _buildInfoItem(
                        'From',
                        item.otFrom.substring(0, 5),
                        LucideIcons.clock,
                      ),
                      const SizedBox(width: 24),
                      _buildInfoItem(
                        'To',
                        item.otTo.substring(0, 5),
                        LucideIcons.clock,
                      ),
                      const SizedBox(width: 24),
                      _buildInfoItem(
            ...
            if (item.status == OvertimeStatus.pending &&
                      ...
                      onPressed: () {
                        showDialog(
                          ...
                          builder: (context) => ShadDialog(
                            title: const Text('Delete Overtime Request'),
